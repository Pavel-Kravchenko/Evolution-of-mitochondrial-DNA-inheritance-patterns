#----------------------------------------------------------------
args <- commandArgs(trailingOnly = T) #collecting input
#----------------------------------------------------------------

name_plot <- toString(args[4]) #making vars
...
name_plot0.0 <- name_plot %+% "/LD.txt" #creating plots names 
...
wind <- as.integer(round(length(x)/10, 0)) #calculating window parameter for moving window average 
if (wind == 0) {wind <- 2}
...
df1 <- data.frame(x, y) #Kendall test
